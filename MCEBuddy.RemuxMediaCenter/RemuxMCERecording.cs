...
namespace MCEBuddy.RemuxMediaCenter
{
    public class RemuxMCERecording
    {
        ...
        public RemuxMCERecording(string Profile, string RecordingFile, string DestinationPath, string audioLanguage, ref JobStatus jobStatus, Log jobLog)
        {
            ...
            _useRemuxsupp = configProfileIni.ReadBoolean(Profile, "UseWTVRemuxsupp", false); // Some videos fail with FFMPEG remuxing and Mencoder encoding (use remuxsupp for remuxing there)
        ...
        private bool RemuxDVRMSFFmpeg()
        {
            ...
            string ffmpegParams = "-fflags +genpts -y -i " + Util.FilePaths.FixSpaces(_RecordingFile) + " -vcodec copy -acodec copy -f mpegts " + Util.FilePaths.FixSpaces(RemuxedFile);
            ...
            {
                _jobLog.WriteEntry(Localise.GetPhrase("DVRMS ReMux using FFMPEG GenPTS failed at") + " " + _jobStatus.PercentageComplete.ToString(System.Globalization.CultureInfo.InvariantCulture) + "%. Retrying using special DVRMS FFMpeg", Log.LogEntryType.Warning);

                // DVR-MS supports only one audio stream
                ffmpegParams = "-y -i " + Util.FilePaths.FixSpaces(_RecordingFile) + " -vcodec copy -acodec copy -f mpegts " + Util.FilePaths.FixSpaces(RemuxedFile);

                ffmpeg = new FFmpeg(ffmpegParams, true, ref _jobStatus, _jobLog);
                ffmpeg.Run();
        ...
        private bool RemuxFFmpeg()
        {
            ...
            if (!skipCopyRemux)
            {
                ...
                while (true) // We do this in a loop until all MPEG2 CopyRemux profiles have been consumed
                {
                    ...
                    if (copyRemuxParams.Contains("-i ")) // Check if we have a input file placeholder (useful if we need to put commands before -i)
                    {
                        copyRemuxParams = copyRemuxParams.Replace("-i ", "-i " + Util.FilePaths.FixSpaces(_RecordingFile) + " ");
                        ffmpegParams = "-y " + copyRemuxParams + " " + Util.FilePaths.FixSpaces(RemuxedFile);
                    }
                    else
            ...
            if (!ffmpeg.Success || !RemuxedFileOK() || skipCopyRemux) //check of file is created, outputhandler reports success (Percentage not requires since Success is more accurate), also check if we need to skip the copy remux to slow remux
            {
                ...
                while (true) // Now loop through all the SlowRemux profiles until we succeed or fail
                {
                    ...
                    if (slowRemuxParams.Contains("-i ")) // Check if we have a input file placeholder (useful if we need to put commands before -i)
                    {
                        slowRemuxParams = slowRemuxParams.Replace("-i ", "-i " + Util.FilePaths.FixSpaces(_RecordingFile) + " ");
                        ffmpegParams = "-y " + slowRemuxParams + " " + Util.FilePaths.FixSpaces(RemuxedFile);
                    }
                    else
                        ffmpegParams = "-y -i " + Util.FilePaths.FixSpaces(_RecordingFile) + " " + slowRemuxParams + " " + Util.FilePaths.FixSpaces(RemuxedFile);
                    ...
                    if (ffmpeg.Success && RemuxedFileOK())
                    {
                        ...
                        if (!ZeroChannelCheckRemuxFFMPEG(slowRemuxParams))
                            ...
                            break; // All done, we're good here
                    }

                    profileCount++; // Move onto the next profile iteration

                    fixCorruptedRemux = true; // We have fixed a corrupted video file through ReMux (not the 1st profile, but subsequent ones)
                }

                if (!ffmpeg.Success || !RemuxedFileOK()) //check of file is created, outputhandler reports success (Percentage not requires since Success is more accurate)
                {
                    _jobLog.WriteEntry(Localise.GetPhrase("Slow Remux using FFMPEG failed at") + " " + _jobStatus.PercentageComplete.ToString(System.Globalization.CultureInfo.InvariantCulture) + "%", Log.LogEntryType.Error);
                    Util.FileIO.TryFileDelete(RemuxedFile);
