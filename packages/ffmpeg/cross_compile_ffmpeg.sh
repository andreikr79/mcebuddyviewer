...
build_ffmpeg() {
  ...
  # FFmpeg 
  local extra_configure_opts="--enable-libsoxr --enable-fontconfig --enable-libass --enable-libutvideo --enable-libbluray --enable-iconv --enable-libtwolame --extra-cflags=-DLIBTWOLAME_STATIC --enable-libzvbi --enable-libcaca --enable-libmodplug --extra-libs=-lstdc++ --extra-libs=-lpng --enable-libvidstab" # extra options: --enable-opencl
...
gcc_cpu_count=4 # allow them to specify more than 1, but default to the one that's most compatible...
...
if [[ $build_ffmpeg_static = "y" ]]; then
  ...
  echo "Creating ffmpeg static distro"
  ./collect.sh || exit 1
  echo "Created ffmpeg static distro (32bit and 64bit) in $dir"
fi
