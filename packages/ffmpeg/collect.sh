...
cp ./sandbox/win32/ffmpeg_git/ffprobe.exe "$dir/32-bit"
...
cp ./sandbox/x86_64/ffmpeg_git/ffprobe.exe "$dir/64-bit"
...
#7zr -mx=1 a "$file.7z" "$file-ffmpeg-static/*" || 7za a "$file.7z" "$file-ffmpeg-static/*"  # some have a package with only 7za, see https://github.com/rdp/ffmpeg-windows-build-helpers/issues/16
#cd ..

#echo "created distros/$file.7z"

echo "Copying to virtual mounted directory"
cp -R -u distros/ /mnt/hgfs/Temp/
