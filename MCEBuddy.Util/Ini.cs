...
namespace MCEBuddy.Util
{
    public class Ini
    {
        ...
        static extern uint GetPrivateProfileSection(string lpAppName, byte[] lpszReturnBuffer, int nSize, string lpFileName);
        ...
        public List<string> GetSectionNames(string PartialSectionName)
        {
            ...
            {
                if (!SectionList[Idx].ToLower().Contains(PartialSectionName.ToLower()))
                {
        ...
        public SortedList<string, string> GetSectionKeyValuePairs(string SectionName)
        {
            ...
            {
                byte[] buffer = new byte[MAX_ENTRY];
                GetPrivateProfileSection(SectionName, buffer, MAX_ENTRY, Filename);
                string[] parts = Encoding.ASCII.GetString(buffer).Trim('\0').Split('\0');
