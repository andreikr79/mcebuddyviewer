...
#region IEnumerable
        ...
        // Start of MCEBuddy Changes

        public string SubTitle
        {
            get
            {
                string Val = GetDescriptorString("WM/SubTitle");
                if (Val != null)
                {
                    return Val.Trim();
                }
            ...
            set { SetDescriptorString(value, "WM/SubTitle"); }
        }
        ...
        public string SubTitleDescription
        {
            get
            {
                string Val = GetDescriptorString("WM/SubTitleDescription");
                if (Val != null)
                {
                    return Val.Trim();
                }
            ...
            set { SetDescriptorString(value, "WM/SubTitleDescription"); }
        }

        public DateTime OriginalBroadcastDateTime
        {
            get
            {
                string OBTStr = GetDescriptorString("WM/MediaOriginalBroadcastDateTime");
                if ((OBTStr == null) || (OBTStr == "")) OBTStr = "1900-01-01T00:00:00Z";
                DateTime OBT;
                if (DateTime.TryParse(OBTStr, out OBT))
                {
                    return OBT;
                }
                else
                {
                    OBTStr = "1900-01-01T00:00:00Z";
                    DateTime.TryParse(OBTStr, out OBT);
                    return OBT;
                }
            }
            set
            {
                string OBTStr = value.ToString("s", System.Globalization.CultureInfo.InvariantCulture);
                SetDescriptorString(OBTStr, "WM/MediaOriginalBroadcastDateTime");
            }
        }
        ...
        public bool CopyProtected
        {
            get
            {
                foreach (ContentDescriptor descriptor in GetDescriptors("WM/WMRVContentProtected"))
                {
                    return descriptor.ToBool();
                }
                return false;
            }
        }

        // End of MCEBuddy Change
		#endregion
	}
}
