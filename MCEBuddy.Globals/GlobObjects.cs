...
namespace MCEBuddy.Globals
{
    public static class GlobalDefs
    {
        ...
        public static string FormatTimeSpan(TimeSpan span, bool showSign, char separator1 = ':', char separator2 = ',')
        {
            string sign = String.Empty;
            if (showSign && (span > TimeSpan.Zero))
                sign = "+";
            return sign +
                   span.Hours.ToString("00") + separator1 +
                   span.Minutes.ToString("00") + separator1 +
                   span.Seconds.ToString("00") + separator2 +
