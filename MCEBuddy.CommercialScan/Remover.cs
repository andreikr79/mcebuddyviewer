...
namespace MCEBuddy.CommercialScan
{
    public class Remover
    {
        ...
        private void CutMencoder()
        {
            ...
            {
                case ".wmv":
                    mencoderParams = Util.FilePaths.FixSpaces(_convertedVideo) + " -of lavf -lavfopts format=asf -ovc copy -oac copy -edl " + Util.FilePaths.FixSpaces(_edlFile) + " -o " + Util.FilePaths.FixSpaces(tempFile);
                    break;
                ...
                case ".ts":
                    ...
                    mencoderParams = Util.FilePaths.FixSpaces(_convertedVideo) + " -of mpeg -ovc copy -oac copy -edl " + Util.FilePaths.FixSpaces(_edlFile) + " -o " + Util.FilePaths.FixSpaces(tempFile);
                    break;
        ...
        private void CutTS()
        {
            ...
            foreach (KeyValuePair<float, float> cut in cutList)
            {
                ...
                // Copy ALL streams including subtitles if any
                cutFFmpeg.Parameters += " -map 0:a -map 0:v -vcodec copy -acodec copy " + Util.FilePaths.FixSpaces(cutFileName);
                cutFFmpeg.Run();
            ...
            aviDemux.Run();
            if(!aviDemux.Success || (Util.FileIO.FileSize(tempFile) <= 0))
            {
                _jobStatus.PercentageComplete = 0;
        ...
        public void StripCommercials()
        {
            ...
            switch (_ext)
            {
                ...
                case ".wmv":
                        ...
                        if (0 == _jobStatus.PercentageComplete) // Try backup
                        {
                            _jobLog.WriteEntry(this, Localise.GetPhrase("CutWMV failed, trying to remove commercials using CutMencoder") + " ext -> " + _ext, Log.LogEntryType.Information);
                            _jobStatus.PercentageComplete = 100; //Reset
                            _jobStatus.ErrorMsg = ""; //reset error message since we are trying again
                            CutMencoder();
                        }
                ...
                case ".mkv":
                    {
                        ...
                        CutMKV(); // TODO: add backup support for mkv using mencoder -of lavf -lavfopts format=matroska
                        break;
                    }

                default:
                    {
                        _jobLog.WriteEntry(this, Localise.GetPhrase("Unsupported extension for removing commercials ") + " " + _ext, Log.LogEntryType.Error);
                        _jobStatus.PercentageComplete = 0;
                        _jobStatus.ErrorMsg = "Unsupported extension for removing commercials";
                        break;
                    }
            }
