...
namespace MCEBuddy.Transcode
{
    public abstract class ConvertBase
    {
            ...
            if (audioDelay != 0)
            {
                ...
                switch (ext)
                {
                    ...
                    case ".m4v":
                            ...
                            ffmpeg.Run();
                            if (!ffmpeg.Success) // Do not check for % completion since FFMPEG doesn't always report a % for this routine for some reason
                            {
                    ...
                    case ".mpg": //TODO: check support for mpg or use mEncoder -of mpeg command
        ...
        private void SetResizeBitRateQuality()
        {
            ...
            // Check for 0 Width, MediaInfo crashes on some .ts files don't report video width, fps or any parameters leaving all 0
            // TODO: Update MediaInfo.dll to latest 2012 version when released with this fix
            if (0 == VideoWidth)
            ...
            _quality = _quality * (float)VideoWidth / (float)DefaultVideoWidth;
        }
        ...
        protected abstract int DefaultVideoWidth { get; }
